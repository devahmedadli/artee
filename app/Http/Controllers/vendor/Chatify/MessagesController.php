...
class MessagesController extends Controller
{
    ...
    {
        // set $id to null if it's not admin for customer and freelancer
        $user = User::find($id);
        if ($user->role != 'admin' && (Auth::user()->role == 'freelancer' || Auth::user()->role == 'customer')) {
            $id = null;
        }
    ...
    public function fetch(Request $request)
    {
        ...
        if ($totalMessages < 1) {
            $response['messages'] = '<p class="message-hint center-el"><span>' . __('Say \'Hi\'') . ' ' . __('and start messaging') . '</span></p>';
    ...
    public function getContacts(Request $request)
    {
        ...
        if ($user->role === 'admin') {
            // Admin can see all customers and freelancers
            $query->whereIn('role', ['customer', 'freelancer']);
        } elseif ($user->role === 'customer' || $user->role === 'freelancer') {
            // Customers and freelancers can only see admins
            $query->where('role', 'admin');
        } else {
            // Handle unexpected role
            return Response::json([
                'contacts' => '<p class="message-hint center-el"><span>Invalid user role</span></p>',
                'total' => 0,
                'last_page' => 1,
            ], 400);
        }

        // Subquery to get the last message date for each user
        $lastMessageQuery = DB::table('ch_messages')
            ->select(
                DB::raw('CASE 
        ...
        // Join with the last message subquery
        $query->leftJoinSub($lastMessageQuery, 'last_message', function ($join) {
            $join->on('users.id', '=', 'last_message.contact_id');
        });

        // Select the required fields and order by last message date (if exists) or user creation date
        $users = $query->select(
            'users.id',
            'users.name',
            'users.email',
        ...
        if (count($usersList) > 0) {
            $contacts = '';
            // dd($usersList);
            foreach ($usersList as $contact) {
                $contacts .= Chatify::getContactItem($contact);
            }
        } else {
