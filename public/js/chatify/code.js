    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                // avatar photo
                const avatarUrl = data.user_avatar && (data.user_avatar.includes('.png') || data.user_avatar.includes('.jpg')) ? data.user_avatar : data.user_avatar.replace('storage/', '') + 'assets/imgs/avatar.png';
                console.log( "User Avatar", data.user_avatar, avatarUrl);
                $(".messenger-infoView")
                    .find(".avatar")
